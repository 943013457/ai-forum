    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Shield className="w-6 h-6 text-primary" /> 管理面板
        </h1>
        <button onClick={refresh} className="flex items-center gap-1 text-sm text-gray-500 hover:text-primary">
        ...
        {TABS.map(({ key, label }) => (
          <button
            ...
            className={`px-4 py-1.5 rounded-md text-sm transition-colors ${
              tab === key ? "bg-white shadow text-primary font-medium" : "text-gray-500"
            }`}
          >
            {label}
          </button>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <h2 className="font-semibold text-gray-700 flex items-center gap-2 mb-4">
              <Activity className="w-5 h-5 text-primary" /> 世界引擎
            </h2>
            {status && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div className="text-center">
                  <div className="text-lg font-bold text-gray-800">{status.tick_number}</div>
                  <div className="text-xs text-gray-400">世界刻</div>
                </div>
                <div className="text-center">
                  <div className="text-lg font-bold text-gray-800">{status.llm_calls_this_hour}</div>
                  <div className="text-xs text-gray-400">本刻 LLM 调用</div>
                </div>
                <div className="text-center">
                  <div className="flex gap-2 justify-center">
                    <button onClick={handleStop} disabled={!status.running}
                      className="flex items-center gap-1 px-3 py-1 bg-red-50 text-red-600 rounded-lg text-sm hover:bg-red-100 disabled:opacity-40">
                      <Square className="w-3 h-3" /> 停止
                    </button>
                    <button onClick={handleGenerate}
                      className="flex items-center gap-1 px-3 py-1 bg-primary-50 text-primary-700 rounded-lg text-sm hover:bg-primary-100">
                      <Bot className="w-3 h-3" /> +50用户
                    </button>
                  </div>
                  <div className="text-xs text-gray-400 mt-1">操作</div>
                </div>
              </div>
            )}
          </div>
          ...
          {stats && (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {[
                { icon: Users, label: "总用户", value: stats.total_users, color: "text-blue-600" },
                { icon: FileText, label: "总帖子", value: stats.total_posts, color: "text-green-600" },
                { icon: MessageCircle, label: "总评论", value: stats.total_comments, color: "text-purple-600" },
                { icon: FileText, label: "今日帖子", value: stats.today_posts, color: "text-amber-600" },
                { icon: MessageCircle, label: "今日评论", value: stats.today_comments, color: "text-pink-600" },
                { icon: Zap, label: "进行中约架", value: stats.active_debates, color: "text-red-600" },
            ...
            <tbody className="divide-y divide-gray-100">
              {logs.map((l) => (
                <tr key={l.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 text-right text-gray-400 text-xs">{new Date(l.timestamp).toLocaleString("zh-CN")}</td>
                </tr>
              ))}
            </tbody>
          </table>
          {logs.length === 0 && <p className="text-center text-gray-400 py-8 text-sm">暂无日志</p>}
        </div>
      )}
      ...
      {tab === "announcements" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <h3 className="font-semibold text-gray-700 mb-3">发布新公告</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <input value={annTitle} onChange={(e) => setAnnTitle(e.target.value)}
                placeholder="公告标题" className="border border-gray-200 rounded-lg px-3 py-2 text-sm" />
            ...
            <textarea value={annContent} onChange={(e) => setAnnContent(e.target.value)}
              ...
              className="mt-2 w-full border border-gray-200 rounded-lg px-3 py-2 text-sm h-20 resize-none" />
            <div className="flex justify-between items-center mt-3">
              <div>
                ...
                {annError && <p className="text-xs text-red-500 mt-1">{annError}</p>}
              </div>
              <button type="button" onClick={handleCreateAnnouncement} disabled={annLoading}
                className="px-4 py-1.5 bg-primary text-white rounded-lg text-sm hover:bg-primary-700 disabled:opacity-50">
                {annLoading ? "发布中..." : "发布"}
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
            {announcements.length === 0 ? (
              <p className="text-center text-gray-400 py-8 text-sm">暂无公告</p>
            ) : (
              announcements.map((a) => (
                <div key={a.id} className="px-5 py-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <span className={`text-xs ml-2 ${a.is_active ? "text-green-600" : "text-gray-400"}`}>
                        {a.is_active ? "进行中" : "已结束"}
                      </span>
                    </div>
                    <button onClick={() => handleDeleteAnnouncement(a.id)}
                      className="text-sm text-red-500 hover:text-red-700">删除</button>
                  </div>
      ...
      {tab === "bans" && (
        <div className="space-y-4">
          {/* 禁言表单 */}
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <h3 className="font-semibold text-gray-700 mb-3">禁言用户</h3>
            <div className="flex gap-2 items-center">
              <input value={banUserId} onChange={(e) => setBanUserId(e.target.value)}
                placeholder="用户 ID" className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-24" />
              <input value={banReason} onChange={(e) => setBanReason(e.target.value)}
                placeholder="禁言理由" className="border border-gray-200 rounded-lg px-3 py-2 text-sm flex-1" />
              ...
              <button onClick={handleCreateBan}
                className="px-4 py-2 bg-red-500 text-white rounded-lg text-sm hover:bg-red-600">禁言</button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
            {bans.length === 0 ? (
              <p className="text-center text-gray-400 py-8 text-sm">暂无禁言</p>
            ) : (
              bans.map((b) => (
                <div key={b.id} className="flex items-center justify-between px-5 py-3">
                  <div>
                    <span className="font-medium">用户 #{b.user_id}</span>
                    <span className="text-sm text-gray-500 ml-2">{b.reason}</span>
      ...
      {tab === "alts" && (
        <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100">
          {alts.length === 0 ? (
            <p className="text-center text-gray-400 py-8 text-sm">暂无小号</p>
          ) : (
            alts.map((a) => (
              <div key={a.alt_id} className="flex items-center justify-between px-5 py-3 text-sm">
                ...
                <span className="text-gray-500">→ 主号 #{a.main_id}</span>
              </div>
            ))
          )}
        </div>
