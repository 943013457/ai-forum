...
function DeleteCountdown({ markedAt }: { markedAt: string }) {
  ...
  if (diff <= 0) return <span className="text-primary-500 text-xs font-medium">即将删除</span>;
  const hours = Math.floor(diff / 60);
  const mins = diff % 60;
  return (
    <span className="inline-flex items-center gap-0.5 text-xs bg-primary-50 text-primary-500 px-1.5 py-0.5 rounded-full font-medium">
...
export default function PostCard({ post }: Props) {
  ...
  return (
    <div className={`bg-dark-card rounded-xl border p-4 hover:bg-dark-hover transition-colors ${
      isMarkedDelete ? "border-primary-300/30" : "border-dark-border"
    }`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5 mb-1 flex-wrap">
            ...
            {post.is_featured && (
              <span className="inline-flex items-center gap-0.5 text-xs bg-amber-900/30 text-amber-400 px-1.5 py-0.5 rounded-full font-medium">
                <Star className="w-3 h-3" /> 精选
              </span>
            )}
            {post.is_rumor && (
              <span className="inline-flex items-center gap-0.5 text-xs bg-orange-900/30 text-orange-400 px-1.5 py-0.5 rounded-full font-medium">
                <AlertTriangle className="w-3 h-3" /> 未证实
              </span>
            )}
            {post.is_poll && (
              <span className="inline-flex items-center gap-0.5 text-xs bg-blue-900/30 text-blue-400 px-1.5 py-0.5 rounded-full font-medium">
                <BarChart3 className="w-3 h-3" /> 投票
              </span>
            )}
            {post.is_debate && (
              <span className="inline-flex items-center gap-0.5 text-xs bg-purple-900/30 text-purple-400 px-1.5 py-0.5 rounded-full font-medium">
                <Swords className="w-3 h-3" /> 约架
              </span>
            )}
            {post.is_repost && (
              <span className="text-xs bg-dark-surface text-dark-muted px-1.5 py-0.5 rounded-full">转发</span>
            )}
          </div>

          {/* title */}
          <Link to={`/post/${post.id}`} className="block">
            <h3 className="font-semibold text-dark-text hover:text-primary-500 transition-colors line-clamp-2">
              {post.title}
            </h3>
          </Link>
          ...
          {/* summary */}
          {post.summary && (
            <p className="text-sm text-dark-muted mt-1 line-clamp-2">{post.summary}</p>
          )}
          ...
          <div className="flex items-center gap-4 mt-3 text-xs text-dark-muted">
            ...
            {post.author && <span className="text-dark-muted">{post.author.username}</span>}
          </div>
        </div>
      </div>
    </div>
